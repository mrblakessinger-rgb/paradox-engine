...
## 0. Plugins (easiest path)

Drop-ins for fleets, queues, APIs, LangGraph, CrewAI:
...
eye = Eye(seed=42, world="fleet")  # or queue / api / auto
...
| Problem | Adapter |
|---------|---------|
| X Agent fleets | `plugins.adapters.fleet.FleetPlugin` |
| Y Worker queues | `plugins.adapters.queue.QueuePlugin` |
| Z API 429 thrash | `plugins.adapters.api_client.ApiClientPlugin` |
| LangGraph | `plugins.adapters.langgraph.eye_gate_node` |
| CrewAI | `plugins.adapters.crewai.EyeStormCrewGuard` |

Full guide: **`plugins/README.md`**  
Smoke: `python -m plugins.examples.minimal_all`
